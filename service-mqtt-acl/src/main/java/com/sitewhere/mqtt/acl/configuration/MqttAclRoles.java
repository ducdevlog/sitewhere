 ...
 * @author Derek
 */
public enum MqttAclRoles implements IConfigurationRoleProvider {

    /**
     * Root device state role.
     */
    MqttAcl(ConfigurationRole.build(
            MqttAclRoleKeys.MqttAcl, "Mqtt Acl", false, false, false, new IRoleKey[]{
                    CommonDatastoreRoleKeys.MqttAcl},
            new IRoleKey[0], true));

    private ConfigurationRole role;

    private MqttAclRoles(ConfigurationRole role) {
        this.role = role;
    }
